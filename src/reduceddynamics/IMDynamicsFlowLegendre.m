function [RDInfo,R,iT,N,T] = IMDynamicsFlowLegendre(etaData,varargin)
% [RDInfo,R,iT,N,T] = IMDynamicsFlowLegendre(etaData)
% Identification of the reduced dynamics in k coordinates, i.e. the vector
% field
%
%                        \dot{x} = R(x)
%
% via a weighted ridge regression on a scaled Legendre basis.
...
%           (1+c1*exp(-c2*t)).^(-1), default 0
% 'l_vals' - regularizer values for the ridge regression, default 1e-4
...
if isempty(options.R_coeff) == 1
    if options.fig_disp_nfp ~= -1
        disp('Estimation of the reduced dynamics in Legendre basis... ')
    end
    ...
    if nCoefs == size(Expmat,1)
        W_full = [zeros(k,1) options.R_coeff]; l_opt = 0; Err = 0;
        ...
        W_leg_fit = zeros(k,size(B_leg,1));
        W_leg_fit(:,activeLegendreRows) = W_leg_fit_active;
    end
end
W_r = W_full(:,2:end);
W_leg = W_full / B_leg;
...
% Matrix B such that normalized scaled Legendre products equal B*phi(x).
% Expmat may include the zero exponent row for the constant term.
...
for iBasis = 1:nBasis
    ...
    for iTerm = 1:size(termExp,1)
        idx = find(ismember(Expmat,termExp(iTerm,:),'rows'),1);
        if ~isempty(idx)
            B(iBasis,idx) = B(iBasis,idx)+termCoeff(iTerm);
        end
    end
end
end
...
% Coefficients in ascending powers of x for P_order((x-center)/scale).
...
options = struct('Style','default','R_PolyOrd', 1,'iT_PolyOrd',1,...
    ...
    'L2',[],'n_folds',0,'l_vals',1e-4,'idx_folds',[],'fold_style',[],...
