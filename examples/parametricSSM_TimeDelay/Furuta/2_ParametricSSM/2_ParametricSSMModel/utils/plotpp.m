function plotpp(odefun, uniTol, varargin)
%% Help for plotpp:
%
...
% alla fine del ciclo, elimina eventuali duplicati quasi coincidenti
if ~isempty(EP_set)
    EP_set = uniquetol(EP_set, uniTol, 'ByRows', true);
    EP_count = size(EP_set, 1);
else
    EP_count = 0;
end

% invio EP_set alla base workspace
assignin('base', 'EP_set',   EP_set);
assignin('base', 'EP_count', EP_count);
...
if EP_count == 0
    ...
    % invio EP_set alla base workspace
    assignin('base', 'SEP_set',   SEP_set);
    assignin('base', 'UEP_set', UEP_set);
end
...
if p.Results.plotEPs
    if ~isempty(SEP_set)
        scatter(SEP_set(:,1),SEP_set(:,2),70, cline,...
               'LineWidth',2.0,'MarkerFaceColor',cline)
    end
    if ~isempty(UEP_set)
        scatter(UEP_set(:,1),UEP_set(:,2),70, cline,...
...
for cnt = 1:numel(sTick)
    pos = find(s >= sTick(cnt), 1);
        ...
        % Se non ho un pos valido o sono già all'ultimo punto, salto
    if isempty(pos) || pos >= numel(x)
        continue;
    end
