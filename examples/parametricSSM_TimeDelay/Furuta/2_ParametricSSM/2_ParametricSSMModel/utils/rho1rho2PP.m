function rho1rho2PP(interpStruct, mu_values, config)
% rho1rho2PP
...
%       config.interpMethodDynamicsPP : 'linear','spline', ...

    outDir  = fullfile(pwd, 'PhasePortraits');
    if ~exist(outDir,'dir'), mkdir(outDir); end
    pdfFile = fullfile(outDir, 'PhasePortraits_all.pdf');

    uniTol      = 6.5e-2;
    TsThreshold = [31.405, 31.68];
    tspan                 = 5000;
    ...
    eigValsTorus = NaN(numel(mu_values), 2);

    for k = 1:numel(mu_values)
        Ts = mu_values(k);

        % --- Interpolated Vector field and Jacobian -----------------------------------
        ...
        EPs_limCycle    = EPs( xor(isZero1, isZero2), :);          % exactly one zero coord
        ...
        if ~isempty(EPs_bothNonZero)
            hTorusAll = plot(EPs_bothNonZero(:,1), EPs_bothNonZero(:,2), 'o', ...
                'MarkerFaceColor',[0.75, 0.00, 0.75], ...
                'MarkerEdgeColor','k', ...
                'MarkerSize',22,'LineWidth',1.5);
            uistack(hTorusAll, 'top');
        else
            hTorusAll = [];
        end 
