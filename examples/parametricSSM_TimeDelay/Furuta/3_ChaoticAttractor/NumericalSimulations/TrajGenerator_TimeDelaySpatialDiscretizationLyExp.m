...
% Purpose:
%   Simulate Furuta pendulum under delayed and spatially discretized PD 
%   controller trajectories from random initial conditions within a 
%   small-radius hypersphere in the phase space around the fixed point to 
%   be stabilized (i.e. origin), collect results, save them, and plot 
%   trajectories.
%
% Outputs:
%   Saves xData -> ./Data_Ts<Ts>_FurutaIntU/xDataKp_<Kp>_Kd_<Kd>.mat
%   Figures for: theta, theta dot, phi dot (one grid per variable).
% ========================================================================
...
r       = deg2rad(0.1); % IC perturbation radius [rad]
x0_onAttractor = deg2rad([9.5746, 94.3693, 419.2299]); %theta, thetadot, phidot% IC on the chaotic attractor, we will simulate nTraj with IC within an hypersphere around this point
...
param.D2   = 1.5;     % custom damping
param.toll = 0.05;    % stability tolerance [deg]
...
parfor idx = 1:nIC
    [t_out, theta_out, thetadot_out, phidot_out] = ...
        simulateFurutaSingle_output(T_final, intraTs, Ts, param, fixed_Kp, fixed_Kd, r, x0_onAttractor);
...
% -------------------------------------------------------------------------
function [t_out, theta_out, thetadot_out, phidot_out] = simulateFurutaSingle_output(T_final, intraTs, Ts, param, Kp, Kd, r, x0_onAttractor)
    ...
    theta0  = x0_onAttractor(1) + r * 2 * (rand - 0.5);
    phi0    = 0 + r * 2 * (rand - 0.5);
    dtheta0 = x0_onAttractor(2) + r * 2 * (rand - 0.5);
    dphi0   = x0_onAttractor(3) + r * 2 * (rand - 0.5);
